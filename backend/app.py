...
# Allow the deployed Vercel frontend to call the API even if Render has an old
# CORS_ORIGINS environment variable. Authentication remains enforced by JWT.
...
def dashboard_summary():
    ...
    return jsonify({
        "success": True,
        "stats": {"projects": len(projects), "tasks": len(tasks), "completed": completed, "teamMembers": User.query.count()},
        "taskBreakdown": {"done": completed, "inProgress": in_progress, "todo": todo},
        "projects": [project_payload(p) for p in projects[:6]],
        "activity": [{"id": a.id, "action": a.action, "context": a.context, "createdAt": a.created_at.isoformat() if a.created_at else None} for a in activities],
    })
...
def get_activity():
    ...
    return jsonify({"success": True, "activity": [{"id": a.id, "action": a.action, "context": a.context, "createdAt": a.created_at.isoformat() if a.created_at else None} for a in activities]})

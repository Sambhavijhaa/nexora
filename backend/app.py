...
def record_activity(user_id, action, context="", workspace_id=None):
...
def token_response(user):
    ...
    refresh_claims = None
    # JWT-Extended does not expose the generated JTI until decoding the token.
    # The session table is therefore populated by the /refresh path when needed.
    return {"accessToken": access, "refreshToken": refresh, "user": user_payload(user)}
...
# -----------------------------------------------------------------------------
# Request lifecycle, security and error handling
# -----------------------------------------------------------------------------
...
def start_request():
    ...
    if request.path.startswith("/api/auth/"):
        # Authentication endpoints are additionally limited below.
        pass
...
def handle_unexpected_error(exc):
    ...
    return error("Internal server error. Please try again.", 500, "INTERNAL_ERROR")


@jwt.unauthorized_loader
def jwt_missing(reason):
    return error("Authorization token is required.", 401, "AUTH_REQUIRED")


@jwt.invalid_token_loader
def jwt_invalid(reason):
    return error("Invalid authorization token.", 401, "AUTH_INVALID")


@jwt.expired_token_loader
def jwt_expired(header, payload):
    return error("Token expired. Please refresh your session.", 401, "AUTH_EXPIRED")


@jwt.revoked_token_loader
def jwt_revoked(header, payload):
    return error("Session revoked. Please sign in again.", 401, "AUTH_REVOKED")
...
@app.get("/")
def root():
    return ok({"message": "Nexora API is running", "health": "/api/health", "api": "/api"})
...
def openapi():
    return jsonify({
        "openapi": "3.0.3",
        "info": {"title": "Nexora API", "version": "2.0.0"},
        "servers": [{"url": request.host_url.rstrip("/")}],
        "paths": {
            "/api/auth/register": {"post": {"summary": "Create an account and workspace"}},
            "/api/auth/login": {"post": {"summary": "Sign in"}},
            "/api/auth/refresh": {"post": {"summary": "Refresh access token"}},
            "/api/auth/me": {"get": {"summary": "Current user"}},
            "/api/dashboard/summary": {"get": {"summary": "Workspace dashboard"}},
            ...
            "/api/team": {"get": {"summary": "List workspace members"}},
            "/api/analytics": {"get": {"summary": "Workspace analytics"}},
            "/api/activity": {"get": {"summary": "Audit activity"}},
            "/api/notifications": {"get": {"summary": "User notifications"}},
        },
    })
...
def register():
    ...
    workspace_name = clean_string(data.get("workspaceName") or f"{name}'s Workspace", 160)

    if len(name) < 2 or not EMAIL_RE.match(email):
        return error("Enter a valid name and email address.", 400, "VALIDATION_ERROR")
    password_error = validate_password(password)
    if password_error:
        return error(password_error, 400, "WEAK_PASSWORD")
    ...
    try:
        ...
        workspace = Workspace(name=workspace_name, slug=slugify(workspace_name), owner_id=user.id)
        db.session.add(workspace)
        db.session.flush()
        db.session.add(Membership(workspace_id=workspace.id, user_id=user.id, role="Admin"))
        record_activity(user.id, "Created the workspace", workspace.name)
        db.session.commit()
        response = token_response(user)
        response["workspace"] = {"id": workspace.id, "name": workspace.name, "slug": workspace.slug, "role": "Admin"}
...
def login():
    ...
    # Backfill workspace membership for users created before the SaaS workspace model.
    ensure_user_workspace(user)
    db.session.commit()
    response = token_response(user)
    workspace = workspace_for_user(user.id)
    response["workspace"] = {"id": workspace.id, "name": workspace.name, "slug": workspace.slug, "role": membership_for(user.id).role}
    return ok(response)
...
def refresh():
    ...
    if not user:
        return error("User not found.", 401, "AUTH_INVALID")
...
@jwt_required()
def logout():
    # Access tokens remain short-lived. A production client should also remove
    # local tokens; this endpoint records the logout event for the audit trail.
    user_id = int(get_jwt_identity())
    record_activity(user_id, "Signed out", "Authentication")
    db.session.commit()
...
def me():
    ...
    ensure_user_workspace(user)
    membership, workspace = current_workspace_context(user.id)
    db.session.commit()
    return ok({
        "user": user_payload(user),
        "workspace": {"id": workspace.id, "name": workspace.name, "slug": workspace.slug, "role": membership.role} if workspace else None,
    })
...
def get_workspace():
    user_id = int(get_jwt_identity())
    workspace = ensure_user_workspace(db.session.get(User, user_id))
    db.session.commit()
    membership = membership_for(user_id, workspace.id)
...
def update_workspace():
    ...
    if len(name) < 2:
        return error("Workspace name is required.", 400, "VALIDATION_ERROR")
    g.workspace.name = name
    record_activity(int(get_jwt_identity()), "Updated workspace settings", name)
    db.session.commit()
    return ok({"workspace": {"id": g.workspace.id, "name": g.workspace.name, "slug": g.workspace.slug, "role": g.membership.role}})
...
def invite_member():
    ...
    if not EMAIL_RE.match(email) or role not in ROLES:
    ...
    if existing:
        notify(existing.id, "Workspace invitation", f"You have been invited to {g.workspace.name} as {role}.", "invite")
    record_activity(int(get_jwt_identity()), "Invited a workspace member", email)
    db.session.commit()
    return ok({"invitation": {"id": invitation.id, "email": email, "role": role, "expiresAt": invitation.expires_at.isoformat()}} , 201)
...
def accept_invitation():
    data = request.get_json(silent=True) or {}
    token = clean_string(data.get("token"), 128)
    ...
    if not invitation or invitation.accepted_at or invitation.expires_at < now_utc():
        return error("This invitation is invalid or expired.", 400, "INVITATION_INVALID")
    user_id = int(get_jwt_identity())
    user = db.session.get(User, user_id)
    if user.email.lower() != invitation.email.lower():
        return error("This invitation was sent to a different email address.", 403, "INVITATION_EMAIL_MISMATCH")
    existing = Membership.query.filter_by(workspace_id=invitation.workspace_id, user_id=user_id).first()
    if not existing:
        db.session.add(Membership(workspace_id=invitation.workspace_id, user_id=user_id, role=invitation.role))
    invitation.accepted_at = now_utc()
    record_activity(user_id, "Joined the workspace", g.workspace.name if getattr(g, "workspace", None) else str(invitation.workspace_id))
    db.session.commit()
...
def update_member_role(member_id):
    ...
    if not membership:
        return error("Team member not found.", 404, "MEMBER_NOT_FOUND")
    data = request.get_json(silent=True) or {}
    role = clean_string(data.get("role"), 40)
    if role not in ROLES:
        return error("Invalid role.", 400, "VALIDATION_ERROR")
    if member_id == g.workspace.owner_id and role != "Admin":
        return error("The workspace owner must remain an Admin.", 400, "OWNER_ROLE_REQUIRED")
    membership.role = role
    record_activity(int(get_jwt_identity()), "Changed team member role", f"{member_id} → {role}")
    db.session.commit()
    return ok({"member": {**user_payload(db.session.get(User, member_id)), "role": role}})


@app.delete("/api/team/<int:member_id>")
@require_role("Admin")
def remove_member(member_id):
    if member_id == g.workspace.owner_id:
        return error("The workspace owner cannot be removed.", 400, "OWNER_PROTECTED")
    membership = Membership.query.filter_by(workspace_id=g.workspace.id, user_id=member_id).first()
    if not membership:
        return error("Team member not found.", 404, "MEMBER_NOT_FOUND")
    db.session.delete(membership)
    record_activity(int(get_jwt_identity()), "Removed a team member", str(member_id))
    db.session.commit()
...
def create_project():
    ...
    try:
        ...
        return error("Progress must be a number from 0 to 100.", 400, "VALIDATION_ERROR")

    user_id = int(get_jwt_identity())
    project = Project(name=name, description=clean_string(data.get("description"), 5000), status=status, progress=progress, owner_id=user_id)
    ...
    record_activity(user_id, "Created a project", name)
    db.session.commit()
...
def update_project(project_id):
    ...
    if not membership or membership.role == "Viewer":
        return error("You do not have permission to edit projects.", 403, "FORBIDDEN")
    project = project_in_workspace(project_id, workspace.id)
    if not project:
        return error("Project not found.", 404, "PROJECT_NOT_FOUND")
    data = request.get_json(silent=True) or {}
    if "name" in data and clean_string(data["name"], 160):
        project.name = clean_string(data["name"], 160)
    if "description" in data:
        project.description = clean_string(data["description"], 5000)
    if "status" in data:
    ...
    if "progress" in data:
        try:
            ...
            return error("Progress must be a number from 0 to 100.", 400, "VALIDATION_ERROR")
    record_activity(user_id, "Updated a project", project.name)
    db.session.commit()
    return ok({"project": project_payload(project, workspace.id)})
...
def delete_project(project_id):
    ...
    if not project:
        return error("Project not found.", 404, "PROJECT_NOT_FOUND")
    record_activity(int(get_jwt_identity()), "Deleted a project", project.name)
    db.session.delete(project)
    db.session.commit()
    return ok({"message": "Project deleted."})

# -----------------------------------------------------------------------------
# Tasks, comments and assignment
# -----------------------------------------------------------------------------
...
def create_task():
    ...
    data = request.get_json(silent=True) or {}
    title = clean_string(data.get("title"), 180)
    if not title or not data.get("projectId"):
        return error("Task title and project are required.", 400, "VALIDATION_ERROR")
    try:
        project_id = int(data.get("projectId"))
    except (TypeError, ValueError):
        return error("Invalid project.", 400, "VALIDATION_ERROR")
    project = project_in_workspace(project_id, workspace.id)
    if not project:
        return error("Project not found.", 404, "PROJECT_NOT_FOUND")
    ...
    if status not in TASK_STATUSES or priority not in TASK_PRIORITIES:
        return error("Invalid task status or priority.", 400, "VALIDATION_ERROR")
    ...
    if assignee_id:
        try:
            ...
            return error("Invalid assignee.", 400, "VALIDATION_ERROR")
        if not Membership.query.filter_by(workspace_id=workspace.id, user_id=assignee_id).first():
            return error("Assignee is not a workspace member.", 400, "INVALID_ASSIGNEE")

    task = Task(title=title, description=clean_string(data.get("description"), 10000), status=status, priority=priority, project_id=project.id, assignee_id=assignee_id)
    db.session.add(task)
    db.session.flush()
    due_date = None
    if data.get("dueDate"):
        try:
            due_date = datetime.fromisoformat(str(data["dueDate"]).replace("Z", "+00:00")).replace(tzinfo=None)
        except ValueError:
            return error("Invalid due date.", 400, "VALIDATION_ERROR")
    labels = data.get("labels") or []
    if not isinstance(labels, list):
        labels = [str(labels)]
    db.session.add(TaskMeta(task_id=task.id, due_date=due_date, labels=",".join(clean_string(x, 40) for x in labels[:10])))
    db.session.add(ProjectMember(project_id=project.id, user_id=user_id)) if not ProjectMember.query.filter_by(project_id=project.id, user_id=user_id).first() else None
    record_activity(user_id, "Created a task", task.title)
    if assignee_id and assignee_id != user_id:
        notify(assignee_id, "New task assigned", f"You were assigned '{task.title}' in {project.name}.", "task")
    db.session.commit()
...
def update_task(task_id):
    user_id = int(get_jwt_identity())
    membership, workspace = current_workspace_context(user_id)
    task = task_in_workspace(task_id, workspace.id) if workspace else None
    if not task:
        return error("Task not found.", 404, "TASK_NOT_FOUND")
    if membership.role == "Viewer":
        return error("You do not have permission to update tasks.", 403, "FORBIDDEN")
    data = request.get_json(silent=True) or {}
    if "status" in data:
        status = clean_string(data["status"], 40)
        if status not in TASK_STATUSES:
            return error("Invalid task status.", 400, "VALIDATION_ERROR")
        task.status = status
    if "priority" in data:
        priority = clean_string(data["priority"], 30)
        if priority not in TASK_PRIORITIES:
            return error("Invalid task priority.", 400, "VALIDATION_ERROR")
        task.priority = priority
    if "title" in data and clean_string(data["title"], 180):
        task.title = clean_string(data["title"], 180)
    if "description" in data:
        task.description = clean_string(data["description"], 10000)
    ...
    meta = TaskMeta.query.filter_by(task_id=task.id).first() or TaskMeta(task_id=task.id)
    if not meta.id:
        db.session.add(meta)
    if "dueDate" in data:
        if data["dueDate"]:
            try:
                meta.due_date = datetime.fromisoformat(str(data["dueDate"]).replace("Z", "+00:00")).replace(tzinfo=None)
            except ValueError:
                return error("Invalid due date.", 400, "VALIDATION_ERROR")
        else:
            meta.due_date = None
    if "labels" in data:
        labels = data["labels"] if isinstance(data["labels"], list) else [data["labels"]]
        meta.labels = ",".join(clean_string(x, 40) for x in labels[:10])
    record_activity(user_id, "Updated a task", task.title)
    if task.assignee_id and task.assignee_id != user_id:
        notify(task.assignee_id, "Task updated", f"'{task.title}' was updated.", "task")
    db.session.commit()
    refresh_project_progress(task.project_id)
    db.session.commit()
    return ok({"task": task_payload(task)})
...
def delete_task(task_id):
    ...
    if membership.role not in {"Admin", "Manager"} and task.assignee_id != user_id:
        return error("You do not have permission to delete this task.", 403, "FORBIDDEN")
    project_id = task.project_id
    record_activity(user_id, "Deleted a task", task.title)
...
def get_comments(task_id):
    ...
    return ok({"comments": [{"id": c.id, "body": c.body, "user": user_payload(db.session.get(User, c.user_id)), "createdAt": c.created_at.isoformat() if c.created_at else None} for c in comments]})
...
def add_comment(task_id):
    ...
    if not task:
        return error("Task not found.", 404, "TASK_NOT_FOUND")
    body = clean_string((request.get_json(silent=True) or {}).get("body"), 5000)
    if not body:
        return error("Comment cannot be empty.", 400, "VALIDATION_ERROR")
    comment = Comment(task_id=task.id, user_id=user_id, body=body)
    db.session.add(comment)
    record_activity(user_id, "Commented on a task", task.title)
    if task.assignee_id and task.assignee_id != user_id:
        notify(task.assignee_id, "New task comment", f"Someone commented on '{task.title}'.", "comment")
    db.session.commit()
    return ok({"comment": {"id": comment.id, "body": comment.body, "user": user_payload(db.session.get(User, user_id)), "createdAt": comment.created_at.isoformat() if comment.created_at else None}}, 201)

# -----------------------------------------------------------------------------
# Dashboard, analytics, activity, notifications
# -----------------------------------------------------------------------------
...
def dashboard_summary():
    ...
    if not membership:
        return error("Workspace not found.", 404, "WORKSPACE_NOT_FOUND")
    project_ids = [x[0] for x in db.session.query(ProjectWorkspace.project_id).filter_by(workspace_id=workspace.id).all()]
    ...
    for task in tasks:
        meta = TaskMeta.query.filter_by(task_id=task.id).first()
        if meta and meta.due_date and meta.due_date < now_utc() and task.status != "Done":
            overdue += 1
    projects = Project.query.filter(Project.id.in_(project_ids)).order_by(Project.created_at.desc()).all() if project_ids else []
    activities = Activity.query.join(Membership, Membership.user_id == Activity.user_id).filter(Membership.workspace_id == workspace.id).order_by(Activity.created_at.desc()).limit(8).all()
    member_count = Membership.query.filter_by(workspace_id=workspace.id).count()
    return ok({
        "stats": {"projects": len(projects), "tasks": len(tasks), "completed": completed, "teamMembers": member_count, "overdue": overdue},
        "taskBreakdown": {"done": completed, "inProgress": in_progress, "todo": todo, "review": sum(1 for t in tasks if t.status == "Review"), "blocked": sum(1 for t in tasks if t.status == "Blocked")},
        "projects": [project_payload(p, workspace.id) for p in projects[:6]],
...
def analytics():
    ...
    overdue = 0
    for t in tasks:
        meta = TaskMeta.query.filter_by(task_id=t.id).first()
        if meta and meta.due_date and meta.due_date < now_utc() and t.status != "Done":
            overdue += 1
    status_counts = {status: sum(t.status == status for t in tasks) for status in TASK_STATUSES}
    priority_counts = {priority: sum(t.priority == priority for t in tasks) for priority in TASK_PRIORITIES}
    ...
    return ok({
        ...
        "projectPerformance": [project_payload(p, workspace.id) for p in projects],
    })
...
def get_activity():
    ...
    return ok({"activity": [activity_payload(a) for a in activities]})
...
def delete_activity(activity_id):
    ...
    activity = (
        ...
        .join(
            Membership,
            Membership.user_id == Activity.user_id
        )
        .filter(
    ...
    if not membership:
        return error("Workspace not found.", 404, "WORKSPACE_NOT_FOUND")
    activities = (
        Activity.query.join(Membership, Membership.user_id == Activity.user_id)
        .filter(Membership.workspace_id == workspace.id)
...
def get_notifications():
    ...
    return ok({"notifications": [notification_payload(n) for n in items], "unread": unread})
...
def read_notification(notification_id):
    ...
    if not item:
        return error("Notification not found.", 404, "NOTIFICATION_NOT_FOUND")
    item.read_at = now_utc()
    db.session.commit()
    return ok({"notification": notification_payload(item)})


@app.post("/api/notifications/read-all")
@jwt_required()
def read_all_notifications():
    Notification.query.filter_by(user_id=int(get_jwt_identity()), read_at=None).update({"read_at": now_utc()}, synchronize_session=False)
    db.session.commit()
...
def bootstrap_legacy_data():
    """Create a workspace for legacy users and connect legacy projects."""
    users = User.query.all()
    changed = False
    for user in users:
        workspace = ensure_user_workspace(user)
        project_links = {x.project_id for x in ProjectWorkspace.query.filter_by(workspace_id=workspace.id).all()}

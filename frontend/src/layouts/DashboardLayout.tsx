...
import { Bell, Menu, Repeat2, X } from "lucide-react";
...
export default function DashboardLayout(){
 ...
 return <div className="app-shell">
  <style>{`.workspace-switcher{position:relative;margin:10px 8px 7px}.workspace-trigger{width:100%;display:flex;align-items:center;justify-content:space-between;gap:8px;padding:11px 10px;border:1px solid #e1e4e8;border-radius:9px;background:#f7f8fa;color:#252936;cursor:pointer;text-align:left}.workspace-trigger strong{display:block;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.workspace-menu{position:absolute;z-index:1200;left:0;right:0;top:calc(100% + 6px);padding:5px;background:#fff;border:1px solid #e1e4e8;border-radius:10px;box-shadow:0 12px 30px rgba(23,28,38,.14);max-height:280px;overflow:auto}.workspace-option{width:100%;display:flex;justify-content:space-between;gap:8px;padding:10px;border:0;border-radius:7px;background:transparent;color:#343944;text-align:left;cursor:pointer}.workspace-option:hover,.workspace-option.current{background:#eeecff;color:#4338a8}.workspace-option strong{display:block;font-size:13px}.workspace-option span{font-size:11px;color:#7a818d}.mobile-nav-close{display:none}@media(max-width:700px){.mobile-menu-button,.mobile-nav-close{display:flex}.mobile-nav-overlay{display:${mobile?"block":"none"};position:fixed;inset:0;background:rgba(17,20,27,.35);z-index:999}.sidebar{width:280px!important;min-width:280px!important;position:fixed!important;left:0;top:0;height:100svh;z-index:1000;overflow:auto;transform:translateX(${mobile?"0":"-105%"});transition:transform .22s ease;background:#fff!important}.mobile-nav-close{position:absolute;right:12px;top:12px;width:34px;height:34px;align-items:center;justify-content:center;border:1px solid #e1e4e8;border-radius:8px;background:#fff}.sidebar-nav{display:flex!important;flex-direction:column!important;gap:5px!important}.nav-label{display:block!important;color:#7a818d!important;font-size:11px!important;margin:10px 8px 5px!important}.nav-item{display:flex!important;width:100%!important;box-sizing:border-box!important;padding:12px 10px!important}.workspace-menu{position:relative;top:auto;left:auto;right:auto;margin-top:6px;box-shadow:none}.main-content{width:100%;min-width:0}.mobile-menu-button{width:34px;height:34px;align-items:center;justify-content:center;border:1px solid #e1e4e8;border-radius:9px;background:#fff}.page-content{padding:20px 14px 36px}}`}</style>
  <div className="mobile-nav-overlay" onClick={()=>setMobile(false)}/>
  <aside className="sidebar">
   <button className="mobile-nav-close" onClick={()=>setMobile(false)} aria-label="Close"><X size={20}/></button>
   ...
   <nav className="sidebar-nav"><p className="nav-label">Workspace</p>{navigation.map(({name,path})=><NavLink key={path} to={path} onClick={()=>setMobile(false)} className={({isActive})=>`nav-item ${isActive?"active":""}`}><span>{name}</span></NavLink>)}<p className="nav-label">Account</p><NavLink to="/notifications" className="nav-item"><span>Notifications</span></NavLink><NavLink to="/settings" className="nav-item"><span>Settings</span></NavLink><button className="nav-item" onClick={logout}><span>Sign out</span></button></nav>

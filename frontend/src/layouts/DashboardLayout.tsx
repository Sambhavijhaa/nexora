...
export default function DashboardLayout() {
  const navigate = useNavigate(); const user = readUser(); const userInitials = initials(user.name || "Nexora"); const [mobileNavOpen, setMobileNavOpen] = useState(false); const [workspaceName, setWorkspaceName] = useState("Your workspace");
  useEffect(() => { api.get("/workspace").then(({ data }) => { const name = data.workspace?.name || data.workspace?.workspaceName; if (name) setWorkspaceName(name); }).catch(() => {}); }, []);
  ...
  return <div className="app-shell">
    <style>{`
      ...
      .workspace-context{margin:10px 8px 6px;padding:10px 11px;border:1px solid #e1e4e8;border-radius:9px;background:#f7f8fa}.workspace-context small{display:block;color:#7a818d;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em}.workspace-context strong{display:block;color:#252936;font-size:13px;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      @media(max-width:700px){
        ...
        .workspace-context{margin:10px 8px 7px}.logout-nav{text-align:left!important;font-family:inherit!important;cursor:pointer}.sidebar-user{border-top:1px solid #e1e4e8!important}.user-copy strong{color:#252936!important}.user-copy span{color:#737985!important}
    ...
    <aside className="sidebar" aria-label="Primary navigation">
      ...
      <div className="workspace-context"><small>Workspace</small><strong>{workspaceName}</strong></div>

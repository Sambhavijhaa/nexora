...
export default function DashboardLayout(){
 ...
 function select(w:Workspace){
  // Workspace selection is client-side context. The API reads X-Workspace-ID
  // on every request, so no separate /select endpoint is required.
 ...
 return <div className="app-shell">
  <style>{`.workspace-switcher{position:relative;margin:10px 8px 14px}.workspace-trigger{width:100%;display:flex;align-items:center;justify-content:space-between;gap:8px;padding:11px 10px;border:1px solid #e1e4e8;border-radius:9px;background:#f7f8fa;color:#252936;cursor:pointer;text-align:left}.workspace-trigger strong{display:block;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.workspace-menu{position:absolute;z-index:1200;left:0;right:0;top:calc(100% + 6px);padding:5px;background:#fff;border:1px solid #e1e4e8;border-radius:10px;box-shadow:0 12px 30px rgba(23,28,38,.14);max-height:280px;overflow:auto}.workspace-option{width:100%;display:flex;justify-content:space-between;gap:8px;padding:10px;border:0;border-radius:7px;background:transparent;color:#343944;text-align:left;cursor:pointer}.workspace-option:hover,.workspace-option.current{background:#eeecff;color:#4338a8}.workspace-option strong{display:block;font-size:13px}.workspace-option span{font-size:11px;color:#7a818d}@media(max-width:700px){.sidebar{width:280px!important;min-width:280px!important;position:fixed!important;left:0;top:0;height:100svh;z-index:1000;overflow:auto;background:#fff!important}.sidebar-nav{display:flex!important;flex-direction:column!important;gap:5px!important}.nav-label{display:block!important;color:#7a818d!important;font-size:11px!important;margin:10px 8px 5px!important}.nav-item{display:flex!important;width:100%!important;box-sizing:border-box!important;padding:12px 10px!important}.workspace-menu{position:relative;top:auto;left:auto;right:auto;margin-top:6px;box-shadow:none}.main-content{width:100%;min-width:0}.page-content{padding:20px 14px 36px}}`}</style>
  <aside className="sidebar">
   ...
   <div className="workspace-switcher">
    ...
    {open&&<div className="workspace-menu">{workspaces.map(w=><button key={w.id} className={`workspace-option ${workspace?.id===w.id?"current":""}`} onClick={()=>select(w)}><div><strong>{w.name}</strong><span>{w.role}</span></div><span>{workspace?.id===w.id?"Current":"Switch"}</span></button>)}</div>}
   </div>

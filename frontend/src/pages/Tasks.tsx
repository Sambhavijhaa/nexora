...
export default function Tasks() {
  ...
  async function changeStatus(task: Task) {
    ...
    await api.patch(`/tasks/${task.id}`, { status: next }); load();
  }
            ...
            {tasks.filter(t => t.status === status).map(task => (
              <div className="task-card panel" key={task.id}>
                <div className="task-card-top">
                  <button className="task-status" onClick={() => changeStatus(task)} aria-label={`Move ${task.title}`}>
                    {status === "Done" ? <CheckCircle2 size={15}/> : status === "In Progress" ? <Clock3 size={15}/> : <Circle size={15}/>} 
                  </button>
                  <button className="danger-button" onClick={() => remove(task.id)} aria-label={`Delete ${task.title}`}><Trash2 size={13}/></button>
                </div>
                <strong>{task.title}</strong><span>{task.projectName}</span><em className={`priority ${task.priority.toLowerCase()}`}>{task.priority}</em>
              </div>
            ))}
            {!tasks.filter(t => t.status === status).length && <div className="empty-state">Drop the next piece of work here.</div>}
          </div>
        ))}
      </div>
    </div>
